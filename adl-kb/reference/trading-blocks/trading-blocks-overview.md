---
title: Trading blocks overview
category: trading-blocks
source: https://library.tradingtechnologies.com/adl/trading-blocks/trading-blocks-overview/
---

# Trading blocks overview

> Category: **Trading Blocks** · [KB Home](../../README.md) · [Source](https://library.tradingtechnologies.com/adl/trading-blocks/trading-blocks-overview/)

Trading blocks allow an ADL algo to access markets for retrieving market data and managing orders. Using trading blocks, an algo can:

* Find tradable instruments
* Get real-time market data for instruments
* Retrieve time and sales data
* Submit orders to the market
* Manage working orders
* Monitor the status of markets

[Next PostInstrument block](instrument-block.md)→
