...
inside one of the 13 TT algos in this folder. This tool answers three questions:
    ...
    what reusable parts exist?          python patterns.py
    which one fits what I'm doing?      python patterns.py --search throttle
    what would it cost me to use it?    python patterns.py --show "Mkt Price"
    give me a copy I can paste in       python patterns.py --extract "Mkt Price" -o out.json
...
def cmd_search(rows, term):
    ...
    for p, files in hits:
        ...
        print(f"        details: python patterns.py --show \"{p.name}\"\n")
...
def cmd_show(rows, name):
    ...
    if not matches:
        print(f"No pattern named '{name}'. Run `python patterns.py` for the index.")
        return
    if len(matches) > 1:
    ...
    print(f"\n  python patterns.py --extract \"{p.name}\" -o {_slug(p.name)}.json")
...
def remint(group_block):
    ...
    instance's (see README.md section 3).
...
def cmd_extract(rows, name, out_path):
    ...
    print("  then: python validate.py <your-algo>.adl.json")
    return 0
...
def main():
    ...
    if not K.corpus_files():
        print("No *.adl.json files next to this script.")
        return 1
    rows = dedupe(collect())
