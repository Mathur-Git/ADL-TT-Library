...
production algos** (4,740 blocks, 51 block types) sitting in this folder.
...
GUIDs, and observed property values. Regenerate with `python extract_schema.py`.
...
Sibling docs: [`adl-kb/`](../adl-kb/README.md) documents ADL *semantics* from TT's official
...
> [OPEN-QUESTIONS.md Q1](../OPEN-QUESTIONS.md#q1).**
...
> block's value, which has no AST. Tracked as [Q2](../OPEN-QUESTIONS.md#q2).
...
`python lookup.py --missing` for the live list and the workarounds.
...
containing it, export it into this folder, and re-run `extract_schema.py`.** One export closes the
...
python validate.py mine.adl.json      # check your file
python validate.py --strict mine.adl.json
...
[OPEN-QUESTIONS.md Q1](../OPEN-QUESTIONS.md#q1), which has the ten-minute round-trip test that
...
```bash
python patterns.py                        # index every reusable Group, by adoption cost
python patterns.py --search throttle
python patterns.py --show "Mkt Price"     # ports, contents, and what it needs wired
...
spliced subgraph is a special case of [Q1](../OPEN-QUESTIONS.md#q1).
...
```bash
cd ADL-jsons
python extract_schema.py     # rewrites block-catalog.json
python validate.py           # confirm the corpus still self-tests clean
python test_validate.py      # confirm the checks still bite
```

Drop new `*.adl.json` exports in this folder first — coverage grows with the corpus, and the
