...
waste an import cycle finding out.
    ...
    python validate.py                       # self-test: check all 13 corpus algos
    python validate.py myalgo.adl.json       # check one file
    python validate.py --strict myalgo.json  # treat warnings as failures
    python validate.py --explain C7          # what a check means and why it matters

## The self-test is the point

Every invariant here was derived FROM the corpus, so running `python validate.py` with
