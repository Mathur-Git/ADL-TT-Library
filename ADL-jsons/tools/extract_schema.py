...
Reads every *.adl.json in ../corpus/ and writes ../block-catalog.json: the
...
the corpus, and the unknowns in ADL-JSON-Format-Spec.md section 8 close the same way.
...
def main():
    ...
    if not files:
        raise SystemExit("No *.adl.json files found in ADL-jsons/corpus/.")
    ...
    dest = K.CATALOG_PATH
    # newline="\n": keep the catalog LF-only so a rebuild on Windows is not a 22k-line diff
