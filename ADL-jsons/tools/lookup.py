...
their own ports; only edges reference connector GUIDs).
    ...
    python tools/lookup.py Number             # defId, connectors, properties for one type
    python tools/lookup.py order              # substring search across type names
    python tools/lookup.py --names Stopwatch  # custom instance names TT gave this block
    python tools/lookup.py --props Order tif  # observed values for one property
    python tools/lookup.py --missing          # blocks adl-kb documents but the corpus lacks
...
port count. See ADL-JSON-Format-Spec.md §3.
"""
...
def load():
    if not os.path.exists(CATALOG):
        sys.exit("block-catalog.json not found  -  run: python tools/extract_schema.py")
...
def show(cat, name):
    ...
    if name not in blocks:
        ...
        if not hits:
            if name in KNOWN_ABSENT:
                ...
                print(f"No block type matching '{name}'. Try: python tools/lookup.py --missing")
            return
        if len(hits) > 1:
    ...
    if name == "Group":
        ...
        print("   Mint fresh GUIDs; the lists below are a meaningless aggregate. Spec section 3.")
...
def missing(cat):
    ...
    print("build it on a blank canvas, export into ADL-jsons/corpus/, run extract_schema.py.")
    ...
    print("NOT missing  -  they are values of Math.mathFunction. `python tools/lookup.py Math`.")
