---
title: TT Volume Duration Order
category: Basic Order Entry
source: https://library.tradingtechnologies.com/trade/basic-order-entry/tt-order-types/description-tt-order-types/tt-volume-duration-order/
---
...
specifies the number or percentage of contracts that must trade to trigger a slice to enter the market. The TT
system calculates the child order portions needed to achieve the total order quantity for the total volume.

![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-tt-volume-duration-illustration.png)

## Behaviors

You can configure the following parent and child order behaviors for TT Volume Duration orders:
...
## Setting the interval and quantity of TT Volume Duration child orders

To set the interval and quantity of child orders:

1. In the **Slice Type** field, select either “Interval” or “Disclose”:
   * The **Interval** field sets the traded volume needed to trigger a slice to enter the market. The
     volume is defined by the exchange volume for the contract and may reflect more trades than indicated by
     summing the Last Traded Quantity (LTQ). This is because exchange volume sometimes includes volume from, for
     example, legs of calendar spread trades.

     **Note**: If you set this option, TT calculates the disclosed quantity for each slice until the total
     volume is traded.
   * The **Disclose** field sets the number of contracts to disclose after each interval. If you set
     the disclosed quantity, also select whether to disclose the portion as a set quantity (**Qty**)
     or percentage of the parent order quantity (**%**).

     **Note**: If you set this option, TT calculates the interval for each slice until the total volume is
     traded.

     ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-disclose-config.png)
2. In the **Duration** field, set the total traded volume required to fill the parent order.

![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-child-slice-config.png)

For example, if you enter a 50-lot Volume Duration order with an **Interval** of 100 and a **Duration** of
1000, TT calculates the disclosed quantity of “5” and will submit a new slice after 100 contracts are traded in
the market.
...
2. Set the **Variance** from the dropdown. This example sets the variance to 50%, so each child order
   ...
   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config.png)
3. To see a proposed distribution of the child orders, click ![the magnifying glass button](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/btn-magnify-glass-7.png). **Note**: You must specify the order quantity to see the
   distribution.A flyout shows the quantity of each child order that will be sent when you submit the parent order.

   ![TBD](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config-show-1.png)

   If you want to change the proposed distribution, you can continue to click ![the magnifying glass button](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/btn-magnify-glass-8.png) until you see a distribution you like.

   ![TBD](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config-2.png)
...
4. Submit the order at the desired side and price.

   If you entered a 100 lot order with these settings, the TT Volume Duration parent order would submit an order
   for the first quantity (e.g., 14). Then, a new child order would be entered after each time 10 contracts were
   traded until a total volume of 100 are traded.

## Setting the type and price of the child order

For each set interval, the TT Volume Duration parent order submits native child orders to the market at a specified
...
2. If you select **Limit**, you can optionally set a relative price for the child order using an
   ...
   price level one tick away from the LTP at the beginning of the slice
   interval.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-child-limit-offset-config.png)
...
* **Leave**: TT leaves the resting child order in the market when the next portion is submitted.
  ...
  ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-action-config.png)
* **Payup**: Configures the resting child order to move a set number of ticks into the market. Set the
  ...
  * **At Half Life**: Executes Payup ticks after half of the volume interval.
  ...
  ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-payup-config.png)
* **Go to Market**: Submits the resting child order as a Market order based on the following setting:
  * **At End**: Go to market at the end of the volume interval (default setting).
  * **At Half**: Go to market after half of the volume interval.
  * **Mkt Order Limit**: Sets the number of ticks from LTP to submit the Market order.

    ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-goto-config.png)
* **Merge**: Merges unfilled child orders into a single order after each volume interval.

  ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-merge-config.png)
...
![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-leftover-payup-mdt.png)
...
2. Enable **With a Tick** and set the quantity threshold.
   ...
   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-wat-qty-config.png)
3. Enter the order at the desired price level.If you entered a Buy order at the inside market, the TT Volume Duration Order parent order will reprice its child order one tick when the quantity for the inside Ask falls below 20.

   ![ADD PIC](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-wat-mdt.png)

   For this order:

   * **A** – When the quantity at this level falls below 20…
   * **B** – The TT Volume Duration Order parent order will reprice its child order one tick higher.
...
## Triggering a TT Volume Duration Order parent order at a specific price level

A TT Volume Duration Order order type supports triggers that let you manage when to submit the parent order, which will then enter its child orders. You can set a trigger price, which puts the TT Volume Duration Order parent order on hold until a trade occurs at a specific price level. When the market reaches that price level, the TT Volume Duration Order parent order changes to Working state and submits its first child order. The TT Volume Duration Order order type supports the following types of triggers:
...
* Same/Opposite Side: A trade occurs at the specified price on the specified side of the market relative to the TT Volume Duration Order parent order.

To set a trigger for the TT Volume Duration Order order to begin working when the market trades at a specific price on the same side as your Buy or Sell order, add a same-side If-Touched trigger as follows:

1. Enable **Trigger** to set a trigger for the order.
2. Select **If Touched** from the dropdown to trigger the TT Volume Duration Order order when a trade occurs at the specified price or better.
...
![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-static-trigger-config.png)

If you clicked at any price level on the Buy side, the TT Volume Duration Order parent order would resemble the following.

![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-static-trigger-mdt.png)

Based on the specified parameters, the TT Volume Duration Order parent order is entered into the market as follows:

* The TT Volume Duration Order parent order is placed at the specified trigger price of 178’12.
* The working quantity of the order you entered is 0 and will remain so until the order is triggered.
* When a Bid is filled at the 178’12 trigger price, the TT Volume Duration Order parent order will begin working its child orders at that price level.

## Using Stop and If-Touched trailing triggers to enter a TT Volume Duration Order parent order at a price level that trails the market

Instead of entering your TT Volume Duration Order parent order at a specific price level, you can configure the TT Volume Duration Order parent order to trail the market by setting the number of ticks away from the market to set your trigger.
...
To set a trailing trigger for the TT Volume Duration Order parent order:
...
5. From the **Trigger price** drop-down, select the price type to trail.
   ...
   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-trailing-trigger-config.png)

If you clicked any price level on the Buy side, the TT Volume Duration Order parent order would resemble the following.

![ADD PIC](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-trailing-trigger-mdt.png)

Based on the specified parameters, the TT Volume Duration Order parent order is entered into the market as follows:

* The TT Volume Duration Order parent order is placed at the trigger price of 3086.25.
* The working quantity of the order you entered is 0 and will remain so until the order is triggered.
* When a Bid is filled at the 3086.25 trigger price, the TT Volume Duration Order parent order will begin working its
  child orders at that price level. If the market moves higher, the Stop trigger’s price will not move.

## Setting start and end times for a TT Volume Duration Order parent order

By default, a TT Volume Duration Order order begins working immediately after submission and continues to work until canceled. You can, however, customize when a TT Volume Duration Order parent order begins working and when it stops.

To set start and stop times for a TT Volume Duration Order parent order:

1. For the **Start** setting, click the dropdown arrow and select the desired [start time option](#start-options).

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-start-time-dropdown-7.png)
...
2. Set a start date and time in the calendar selector, and click **Apply**.
   ...
   **Tip**: Use the “tab” key to navigate to the right between time edit boxes. Use “Shift + tab” to navigate left to an edit box.
   ...
   ![ADD PIC](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-times-config.png)

   **Note**: If you specify a future start time, the Order Book will show the TT Volume Duration Order parent order **Status** as **Working** and its **SynthStatus** as **Waiting**.
3. For the **End** setting, click the dropdown arrow and select one of the following:
   * **GTC**: Keeps the TT Volume Duration Order parent order working until it is canceled.
   * **Time**: Sets a time and date for when the TT Volume Duration Order parent order ends.
   * **Day**: Cancels the TT Volume Duration Order parent order at the end of the trading session.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-end-time-dropdown-7.png)
...
4. Enter the order at the desired price level.If you placed the order with a future start time, the TT Volume Duration Order parent order would appear in MD Trader and the Order Book similar to the following.

   ![ADD PIC](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-times-mdt-ob.png)

   The Order Book shows the status of the future order.
   * **A** – The **Status** is **Working**, which indicates the TT Volume Duration Order parent order is working on the Algo Server.
...
## TT Volume Duration order parameters

Configuration parameters include: slice details, child order details, and precondition details.

### Slice details

* **Slice Type** — Select one of the following:
  * **Interval**: The exchange traded volume needed to trigger a slice to enter the market. TT calculates the
    size of each slice needed to fill the parent order quantity based on total traded volume.
  * **Disclose**: The quantity or percentage of contracts to disclose after each interval. If set, TT
    calculates the interval based on total traded volume.

  When the disclosed quantity is specified as a percentage:
...
* **Duration**: The total traded volume required to fill the parent order.
* **Variance**: Sets the percentage (0-100) by which to vary the child order quantity.

### Child order details

* **Child order type**: Sets the type of exchange-native order to use for the order. Possible values
  include:
  * Limit
  * Market
...
←[Previous PostTT Trailing Limit order](tt-trailing-limit-order.md)

[Next PostTT Volume Sliced Order](tt-volume-sliced-order.md)→
...
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-child-slice-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/btn-magnify-glass-7.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config-show-1.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/btn-magnify-glass-8.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-variable-qty-config-2.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-child-limit-offset-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-action-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-payup-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-leftover-goto-config.png
...
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-leftover-payup-mdt.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-wat-qty-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-wat-mdt.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-static-trigger-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-static-trigger-mdt.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-trailing-trigger-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-sliced-trailing-trigger-mdt.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-start-time-dropdown-7.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-times-config.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-end-time-dropdown-7.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/tto-volume-duration-times-mdt-ob.png
