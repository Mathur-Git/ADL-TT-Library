---
title: Organizing the Widgets menu
category: Overview
source: https://library.tradingtechnologies.com/trade/overview/widgets/task-widgets/organizing-the-widgets-menu/
---

# Organizing the Widgets menu

> Category: **Overview** · [Source](https://library.tradingtechnologies.com/trade/overview/widgets/task-widgets/organizing-the-widgets-menu/)
>
> **Interpreted in:** [Platform & Workspace § Widgets](../../../../guides/platform-and-workspace.md#widgets)

TBD Intro

### Selecting favorite widgets for the menu

To add a widget to the favorites section of the menu:

1. Locate the desired widget in the appropriate widget category sub-menu.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-add-widget-1.png)
2. Click the favorite indicator to select it.

   The widget is added to the end of the favorites section of the menu.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-add-widget-2.png)

To remove a widget from the favorites section:

1. Hover the mouse over the right-edge of the widget you want to remove.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-remove-widget-1.png)
2. Click the favorite indicator.

   The widget is removed from the favorites section. However, it is still located in its widget category sub-menu, with its favorite icon disabled.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-remove-widget-2.png)

   **Note**: If you remove a favorite, its icon no longer appears as launch options in the product and instrument search results.

### Rearranging the menu favorites

To rearrange the items in the favorites section:

1. Press and hold the mouse button on the widget you want to move.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-1.png)
2. Drag the widget to the desired location in the favorites section.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-2.png)
3. Release the mouse button.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-3.png)

**Note**: When displaying multiple widget icons in search results, they are shown in the new order they appear in the menu.

←[Previous PostReporting an incident or sending feedback](reporting-an-incident-or-sending-feedback.md)

### Images on page
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-add-widget-1.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-add-widget-2.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-remove-widget-1.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-remove-widget-2.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-1.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-2.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/wgt-menu-rearrange-3.png
