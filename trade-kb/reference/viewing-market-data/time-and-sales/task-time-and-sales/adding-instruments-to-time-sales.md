---
title: Adding instruments to Time & Sales
category: Viewing Market Data
source: https://library.tradingtechnologies.com/trade/viewing-market-data/time-and-sales/task-time-and-sales/adding-instruments-to-time-sales/
---

# Adding instruments to Time & Sales

> Category: **Viewing Market Data** · [KB Home](../../../../README.md) · [Source](https://library.tradingtechnologies.com/trade/viewing-market-data/time-and-sales/task-time-and-sales/adding-instruments-to-time-sales/)

You can populate a Time & Sales widget with one or more products and instruments. You can add a product, a single instrument, multiple instruments, all instruments in a product, or instruments from multiple products. When you add a product to a Time & Sales widget, newly created instruments for the product get added to that widget automatically.

**Note**: A Time & Sales widget supports a maximum of 5,000 instruments.

1. From the context menu, select **Choose Instruments**.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-1.png)
2. Click **Add**, and use the search and explore functionality to locate the instrument or product to add.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-2.png)
3. Check the instruments you want to include, and click **Save**.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-3.png)

   Time & Sales shows activity for all of the selected instruments.

   ![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-4.png)

You can also use your system’s multi-select feature to choose multiple products or instruments in the Market Explorer. The following example uses multi-select to add instruments for the ZB and ZC products.

![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-multi-select.png)

[Next PostLaunching widgets from Time & Sales](launching-widgets-from-time-sales.md)→

### Images on page
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-1.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-2.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-3.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-instr-4.png
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/ts-add-multi-select.png
