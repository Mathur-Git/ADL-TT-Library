---
title: Introduction to Market Grid
category: Viewing Market Data
source: https://library.tradingtechnologies.com/trade/viewing-market-data/market-grid/description-market-grid/introduction-to-market-grid/
---

# Introduction to Market Grid

> Category: **Viewing Market Data** · [KB Home](../../../../README.md) · [Source](https://library.tradingtechnologies.com/trade/viewing-market-data/market-grid/description-market-grid/introduction-to-market-grid/)

The Market Grid displays market data for numerous instruments in a grid-like format and gives you the ability to quickly trade any of those instruments with an order ticket or MD Trader. You can organize the display of instruments to your liking by using tabs. You can also display market depth for an instrument with a simple mouse click.

![](https://library.tradingtechnologies.com/wp-content/uploads/2025/12/mg-overview2.png)

You can open the Market Grid widget as follows:

* Search for a product or instrument in the title bar, select it, then click on the Market Grid icon.
* Click **Explore** in the title bar, select an expiry, then click on the Market Grid icon.
* Select Market Grid from the **Widgets** menu.

←[Previous PostMarket Grid Display](market-grid-display.md)

[Next PostMarket data in Market Grid](market-data-in-market-grid.md)→

### Images on page
- https://library.tradingtechnologies.com/wp-content/uploads/2025/12/mg-overview2.png
